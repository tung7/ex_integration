...
public class ProxyTask implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            if(header.getHost().contains("google.com")) {
                return;
            }
            //添加请求日志信息
            builder.append("\r\n").append("Request Url  ：" + header.getRequestUrl());
            ...
            //读取客户端请求过来的数据转发给服务器
            StringBuilder sb = readForwardDate(isIn, osOut);


            builder.append("\r\n").append("show params  ：" + new String(sb));
            //等待向客户端转发的线程结束
//            ot.join();
     ...
     * @param osOut
     */
    private StringBuilder readForwardDate(InputStream isIn, OutputStream osOut) throws IOException {
